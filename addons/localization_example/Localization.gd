...
func init_connections() -> void:
	...
	var resultQuit = _button_quit_ui.connect("pressed", _on_quit_pressed)
	if resultQuit != OK:
		push_error("Can't connect quit button")
	var resultLang = _button_optins_lang_ui.connect("item_selected", _on_language_item_selected)
	if resultLang != OK:
		push_error("Can't connect lang option button")
	var resultPlayAudio = _button_play_sound_ui.connect("pressed", _on_play_audio_pressed)
	if resultPlayAudio != OK:
		push_error("Can't connect play audio button")
	var resultPlayVideo = _button_play_video_ui.connect("pressed", _on_play_video_pressed)
